...
const getMessages = async (req, res, next) => {
  try {
    ...
    const portfolioQuery =
      "SELECT utp.user_id AS user_id, p.user_id_status_request AS requester_id, u.username AS requester_name, p.request_creation_date AS date, utp.portfolio_id AS portfolio_id, p.name_of_portfolio AS portfolio_name, p.status AS action, p.initial_amount AS initial_amount FROM usertoportfolio utp LEFT JOIN portfolio p ON utp.portfolio_id = p.id LEFT JOIN users u ON p.user_id_status_request = u.id WHERE p.status LIKE '%pending%' AND utp.user_id = $1";
